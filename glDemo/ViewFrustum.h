...
namespace cst {
	...
	private:
		...
		glm::vec4		eLeft, eRight, eTop, eBottom, eNear, eFar;
		...
		glm::vec4		wLeft, wRight, wTop, wBottom, wNear, wFar;
	...
	public:
		...
		ViewFrustum(); // default constructor - initialise projection coefficients to a basic camera model where fovy=55.0f, aspect = 1.0f, d_near = 0.1f and d_far = 500.0f

		ViewFrustum(float fovyInit, float aspectInit, float nearInit, float farInit = 0.0f); // initialise the view frustum and set the projection matrix to the infinite projection matrix if farInit = 0.0 as determined by equalf (this is the default)


		// Accessor methods

		const glm::mat4& projectionMatrix() const; // return a const reference to the (derived) projection matrix
		...
		void getEyeCoordPlanes(glm::vec4* eLeft_, glm::vec4* eRight_, glm::vec4* eTop_, glm::vec4* eBottom_, glm::vec4* eNear_, glm::vec4* eFar_) const; // return the frustum planes in eye coordinates via the specified pointers.  All provided pointers are assumed to be valid

		void getWorldCoordPlanes(glm::vec4* wLeft_, glm::vec4* wRight_, glm::vec4* wTop_, glm::vec4* wBottom_, glm::vec4* wNear_, glm::vec4* wFar_) const; // return the frustum planes in world coordinates via the specified pointers.  All provided pointers are assumed to be valid


		// Frustum plane methods

		void calculateWorldCoordPlanes(const glm::vec4& vcamPos, const glm::mat4& R); // Given camera position C and orientation R in R3, calculate the view frustum planes in world coordinate space.  The frustum planes are normalised so |abc| = 1.0 for the plane coefficients <a, b, c, d>

		bool pointInFrustum(const glm::vec4& P) const; // return true if the given point P lies inside the frustum, false otherwise.  P is assumed to be in world coordinate space and it is assumed the world coordinate planes of the frustum have already been calculated with a previous call to CGViewFrustum::calculateWorldCoordPlanes.  If d_far = 0.0 (an infinite perspective projection is used) then the far plane is ignored in determining whether the point lies within the frustum
