...
GLuint setupShaders(const string& vsPath, const string& gsPath, const string& fsPath, GLSL_ERROR* error_result) {
	...
	const string* vertexShaderSource = NULL;
	const string* geometryShaderSource = NULL;
	const string* fragmentShaderSource = NULL;
	...
	if (gsPath.length() > 0) {
		...
		// Geometry shader specified
		GLSL_ERROR err = createShaderFromFile(GL_GEOMETRY_SHADER, gsPath, &geometryShader, &geometryShaderSource);
